...
export function buildShoppingSections(
  ...
  // Seeded from `categories` so the map iterates in display order and items of
  // an undisplayed category find no bucket and are dropped.
  const byCategory = new Map<TIngredientCategories, ComputedShoppingItem[]>(
    categories.map(category => [category, []])
  );
  const purchased: ComputedShoppingItem[] = [];

  for (const item of items) {
    if (item.purchased) {
      purchased.push(item);
    } else {
      byCategory.get(item.type as TIngredientCategories)?.push(item);
    }
  }

  const sections: ShoppingSection[] = [];
  for (const [title, data] of byCategory) {
    if (data.length > 0) {
      sections.push({ title, data });
    }
  }

  return { sections, purchased };
}
