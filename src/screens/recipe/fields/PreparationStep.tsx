...
export function PreparationStepField({ form, index, t }: PreparationStepFieldProps) {
  ...
  // Commit handlers run on the input's end-editing event, which on a real
  // device coincides with blur. Marking the controller touched lets RHF's
  // `mode: 'onTouched'` re-run validation and open the inline gate the first
  // time the user actually edits the step.
