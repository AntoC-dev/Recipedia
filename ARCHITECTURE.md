...
**Recipe images are bounded before they reach permanent storage.** `saveRecipeImage` runs every picked, captured, or downloaded file through `resizeImageToBound` (`@utils/imageResize`), capping the longest side at `MAX_IMAGE_DIMENSION` while keeping the original container format. An 8–48MP camera file must never land in `documents/Recipedia/` verbatim. This applies to images entering storage from now on; existing libraries are not rewritten, because re-encoding a user's only copy of their own photo is destructive.
