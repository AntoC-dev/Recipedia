...
function checkboxStatus(getByTestId: ReturnType<typeof renderItem>['getByTestId']) {
  return getByTestId(`${testID}::Checkbox::Status`, { includeHiddenElements: true }).props.children;
}
...
describe('ShoppingListItem', () => {
  ...
  test('leaves the checkbox unchecked and the text plain while still to buy', () => {
    ...
    expect(checkboxStatus(getByTestId)).toBe('unchecked');
  ...
  test('checks the box and strikes the text through once purchased', () => {
    ...
    expect(checkboxStatus(getByTestId)).toBe('checked');
  ...
  test('exposes the row as a checkbox to screen readers', () => {
    ...
    expect(getByTestId(testID).props.accessibilityRole).toBe('checkbox');
  });

  test('hides the decorative checkbox from screen readers', () => {
    const { queryByTestId } = renderItem();

    expect(queryByTestId(`${testID}::Checkbox`)).toBeNull();
  });

  test('reports the unpurchased row as unchecked', () => {
    const { getByTestId } = renderItem({ purchased: false });

    expect(getByTestId(testID).props.accessibilityState).toEqual({ checked: false });
  });

  test('reports the purchased row as checked', () => {
    const { getByTestId } = renderItem({ purchased: true });

    expect(getByTestId(testID).props.accessibilityState).toEqual({ checked: true });
  });
});
